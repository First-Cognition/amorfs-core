...
import { parse, isValid, parseOrThrow, ConceptNode } from '../src/index.js';
...
// Helper to get the first parameter value from a concept
function getFirstExpressionValue(concept: ConceptNode): string | number | null {
	if (concept.parameter.length === 0) return null;
	return concept.parameter[0].value;
}

// Helper to find a child association by its first expression value
function findChildByExpression(concept: ConceptNode, exprValue: string): typeof concept.children[number] | undefined {
	return concept.children.find(a => getFirstExpressionValue(a.concept) === exprValue);
}
...
describe('business_card.amorfs', () => {
	...
	it('should have Alan Turing as an expression value', () => {
		...
		expect(concept.parameter).toBeDefined();
    
		// Find Alan Turing in parameter or children
		const hasTuring = concept.parameter.some(e => e.value === 'Alan Turing') ||
      concept.children.some(c => c.concept.parameter?.some(e => e.value === 'Alan Turing'));
	...
	it('should have nested contact and address associations', () => {
		...
		const contactAssoc = findChildByExpression(concept, 'contact');
		const addressAssoc = findChildByExpression(concept, 'address');
    
		expect(contactAssoc).toBeDefined();
		expect(addressAssoc).toBeDefined();
    
		// Check contact has email and phone
		const contactChildren = contactAssoc?.concept.children || [];
		expect(contactChildren.some(a => getFirstExpressionValue(a.concept) === 'email')).toBe(true);
		expect(contactChildren.some(a => getFirstExpressionValue(a.concept) === 'phone')).toBe(true);
	});
});
...
describe('multilingual_product.amorfs', () => {
	...
	it('should have a product concept with key and parameter', () => {
		const ast = parseOrThrow(input);
		expect(ast.concepts).toHaveLength(1);
		expect(getFirstExpressionValue(ast.concepts[0])).toBe('product');
    
		// Should have parameter array (may contain multilingual values)
		const concept = ast.concepts[0];
		expect(concept.parameter).toBeDefined();
		expect(Array.isArray(concept.parameter)).toBe(true);
	});

	it('should have price with intrinsic associations', () => {
		const ast = parseOrThrow(input);
		const concept = ast.concepts[0];
		const priceAssoc = findChildByExpression(concept, 'price');
    
		expect(priceAssoc).toBeDefined();
    
		// Price should have intrinsic associations (amount, currency)
		const priceChildren = priceAssoc?.concept.children || [];
		const amountAssoc = priceChildren.find(a => getFirstExpressionValue(a.concept) === 'amount');
		const currencyAssoc = priceChildren.find(a => getFirstExpressionValue(a.concept) === 'currency');
	...
	it('should have features with contextual associations', () => {
		const ast = parseOrThrow(input);
		const concept = ast.concepts[0];
		const featuresAssoc = findChildByExpression(concept, 'features');
...
describe('person.amorfs', () => {
	...
	it('should have associations with job title', () => {
		...
		const jobTitleAssoc = findChildByExpression(concept, 'job title');
	...
	it('should have intrinsic associations for birth date and nationality', () => {
		...
		const birthDateAssoc = findChildByExpression(concept, 'birth date');
		const nationalityAssoc = findChildByExpression(concept, 'nationality');
...
describe('expression_types.amorfs', () => {
	...
	it('should handle numeric expressions', () => {
		...
		const ageValue = ageConcept?.children.find(a => a.concept.parameter[0]?.value === 42);
...
describe('multiple_expressions.amorfs', () => {
	...
	it('should handle multiple expressions (synonyms)', () => {
		...
		// Should have child with multiple parameter expressions
		const child = stateConcept?.children[0];
		expect(child?.concept.parameter.length).toBeGreaterThanOrEqual(2);
	});
});
...
describe('associations.amorfs', () => {
	...
	it('should distinguish intrinsic and contextual associations', () => {
		...
		// Filter to only has_a associations (not which_is for the value)
		const intrinsicHasA = personIntrinsic?.children.filter(a => a.associationType === 'has_a') || [];
		const contextualHasA = personContextual?.children.filter(a => a.associationType === 'has_a') || [];
...
describe('nested_structures.amorfs', () => {
	...
	it('should handle deeply nested structures', () => {
		...
		// Verify deep nesting exists - location has children with children
		const hasDeepNesting = locationConcept?.children.some(a => 
			a.concept.children && a.concept.children.some(b =>
				b.concept.children && b.concept.children.length > 0
			)
		);
...
describe('units_of_measurement.amorfs', () => {
	...
	it('should handle measurements with units', () => {
		...
		const valueAssoc = findChildByExpression(measurement!, 'value');
		const unitAssoc = findChildByExpression(measurement!, 'unit');
